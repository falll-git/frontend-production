...
export default function TitipanAsuransiPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        if (!ignore) showToast(error instanceof Error ? error.message : "Gagal memuat data titipan asuransi", "error");
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        showToast("Dana titipan asuransi diperbarui", "success");
      } else {
        const created = await legalService.createDeposit(payload);
        setItems((prev) => [created, ...prev]);
        showToast("Dana titipan asuransi ditambahkan", "success");
      }
      closeModal();
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setDeleteItem(null);
      showToast("Dana titipan asuransi dihapus", "success");
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto animate-fade-in space-y-6">
      <FeatureHeader title="Dana Titipan Asuransi" subtitle="Kelola dana titipan untuk keperluan proses asuransi." icon={<Shield />}
        ...
        <div className={SETUP_PAGE_TABLE_SCROLL_CLASS}>
          <table className={`${SETUP_PAGE_TABLE_CLASS} table-fixed w-full`}>
            ...
            <tbody className={SETUP_PAGE_TABLE_BODY_CLASS}>
              ...
              {!isFetching && filtered.length === 0 && <tr><td colSpan={7} className={SETUP_PAGE_EMPTY_STATE_CELL_CLASS}>{getSetupPageEmptyStateCopy("dana titipan asuransi")}</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
        ...
        <div data-dashboard-overlay="true" className="fixed inset-0 p-4" style={{ background: "rgba(0,0,0,0.55)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={closeModal}>
          <div className="bg-white rounded-lg shadow-sm w-full max-w-lg overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <div><h2 className="text-xl font-bold text-gray-900">{editingId ? "Edit Dana Titipan Asuransi" : "Tambah Dana Titipan Asuransi"}</h2></div>
      ...
      <DeleteConfirmModal isOpen={deleteItem !== null} title="Hapus Dana Titipan Asuransi?" entityLabel="dana titipan" itemName={deleteItem?.contract_id ?? deleteItem?.id ?? ""} onClose={() => setDeleteItem(null)} onConfirm={() => void confirmDelete()} isLoading={isDeleting} />
    </div>
  );
}
